 ...
 *     serialise JSON response bodies)
...
export class UsersRestService extends ObjectRestService {
  override async init(): Promise<void> {
    this.handle('GET',    '/users',      this.list)
    this.handle('GET',    '/users/:id',  this.getOne)
    this.handle('POST',   '/users',      this.create)
    this.handle('PUT',    '/users/:id',  this.update)
    this.handle('DELETE', '/users/:id',  this.remove)
  }

  // ── handlers ──────────────────────────────────────────────

  private async list(_req: HttpRequest): Promise<HttpResponse> {
    const users = await B.usersBusiness.list()
  ...
  private async create(req: HttpRequest): Promise<HttpResponse> {
    ...
    const user = await B.usersBusiness.create({ name: dto.name, email: dto.email })
  ...
  private async update(req: HttpRequest): Promise<HttpResponse> {
    ...
    const user = await B.usersBusiness.update(req.params.id!, dto)
    if (user === null) throw new NotFoundException(`User ${req.params.id}`)
    return { status: HttpStatusUtils.OK, body: user }
  }

  private async remove(req: HttpRequest): Promise<HttpResponse> {
    const ok = await B.usersBusiness.delete(req.params.id!)
