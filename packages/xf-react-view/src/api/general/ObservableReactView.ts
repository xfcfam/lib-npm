import { ReactView, type Constructor, type ViewLifecycle } from './ReactView.js'
...
export function ObservableReactView<TState = unknown>() {
  return function observableReactView<TBase extends Constructor>(
    ...
    return class ObservableReactViewGeneralization extends ReactView(Base) {
      ...
      /**
       * Drop all observers, then run the base {@link ReactView} teardown.
       * The React unmount bridge (inherited from {@link ReactView}) invokes
       * this on `componentWillUnmount`, and the injection `A` on shutdown.
       * `init` (the no-op default plus the mount bridge) is inherited from
       * {@link ReactView} unchanged.
       */
