...
export function ReactView<TBase extends Constructor>(Base: TBase) {
  return class ReactViewGeneralization extends Base implements ViewLifecycle {
     ...
     *
     * Put mount logic in {@link init}, not here: a subclass that overrides
     * `componentDidMount` without calling `super.componentDidMount()` would
     * silently drop the `init` bridge. `init` runs fire-and-forget, so a
     * rejection surfaces as an unhandled promise — handle errors inside it.
     */
